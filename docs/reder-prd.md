...
**F2.3** — Shim connections are authenticated by a per-session token generated by `rederd` and retrieved by the shim via command-line argument at spawn time (written into the `.mcp.json` entry by the `reder install` command).
...
**F5.1** — `reder init` — interactive config wizard. Generates `~/.config/reder/reder.config.yaml`, generates machine keys, writes systemd unit, prompts for initial Telegram bot token.

**F5.2** — `reder install <session_id>` — registers a new session. Generates a session token, writes a `.mcp.json` entry into the current project with the shim command and token. Idempotent.
...
**S3 — Per-session shim tokens.** Every `reder-shim` instance authenticates to the daemon with a unique token. Token is generated by `reder install <session_id>`, stored in the project's `.mcp.json` (which is `0600`), and presented on IPC connect. Tokens can be revoked via `reder revoke <session_id>`.
...
- Voice-note STT.
- `reder init`, `reder install`, `reder doctor`, `reder status`, `reder logs`.
...
1. **Claude Code channel plugin distribution.** The official path is `/plugin install`. Can `reder install <session_id>` write an entry into Claude Code's plugin config, or does it have to write only `.mcp.json`? Need to verify what Claude Code 2.1.81+ accepts for custom channels beyond `--dangerously-load-development-channels`.
