...
- **Session activity shows `unknown` in dashboard**: reder's Claude Code hooks aren't installed (or the config drifted). Run `reder sessions repair <id>`. `reder doctor` enumerates which sessions are missing hooks.
